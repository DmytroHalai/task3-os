...
public class Main {
    ...
    private static void runScenario(
        ...
        log.println();
        simulateAccesses(mmu, processes, accessesPerProcess, workingSetChangePeriod, workingSetBias, rnd, log);
        terminateAllProcesses(mmu, processes, log);
        logResults(mmu, title, policy, log);
    }

    private static void simulateAccesses(
            MMU mmu,
        ...
        for (int step = 1; step <= accessesPerProcess; step++) {
            for (Process p : processes) {
                ...
                if (step % workingSetChangePeriod == 0) {
                    p.refreshWorkingSet(processes.size(), rnd);
                }
            }
            round++;
            if (round % 50 == 0) {
                log.printf("\n[Round %d]\n", round);
                logFrameState(mmu, log);
            }
        }
    }

    private static void logFrameState(MMU mmu, PrintWriter log) {
        ...
        for (PhysicalPage pp : mmu.getBusyPhysicalPages()) {
            ...
            log.printf("  Frame %d -> VPage(pid=%d, vpn=%d, pres=%b, ref=%b, mod=%b)%n",
                    pp.getId(),
                    vp.getPid(),
                    vp.getVpn(),
                    vp.isPresent(),
    ...
    private static void terminateAllProcesses(MMU mmu, List<Process> processes, PrintWriter log) {
        for (Process p : processes) {
            mmu.terminateProcess(p);
        }
        log.println("\n[After termination]");
        logFrameState(mmu, log);
    }

    private static void logResults(MMU mmu, String title, ReplacementPolicy policy, PrintWriter log) {
